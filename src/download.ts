...
export class Downloader {
    ...
    public constructor(options: DownloadOptions) {
        ...
        this.rssUrl = `${BASE_URL}/project/${options.project}/rss?path=${(options.path)}`;
    }
    private async getFileList() {
        const responseXml = await fetch(this.rssUrl);
        if (!responseXml.ok) {
        ...
        const files: FileItem[] = new Array(items.length);
